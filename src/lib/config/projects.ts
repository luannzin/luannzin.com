export const PROJECTS = [
  {
    name: "developing something",
    description:
      "i don't have own projects yet, but you can see my freelance projects in the projects tab",
  },
];

export const PAID_PROJECTS = [
  {
    name: "Multi-Tenant Communication ERP",
    description:
      "Architected and developed the frontend of a multi-tenant ERP platform, implementing tenant isolation, caching strategies, and scalable architecture for multiple clients.",
    technologies: ["Next.js", "TypeScript", "Tailwind CSS"],
  },
  {
    name: "Church Management ERP",
    description:
      "Led frontend development of a large-scale ERP platform, delivering and maintaining over 400,000 lines of code across multiple modules and complex business workflows.",
    technologies: ["Next.js", "TypeScript", "Tailwind CSS", "shadcn/ui"],
  },
  {
    name: "Academic ERP for Medical University",
    description:
      "Developed a full-stack academic ERP, implementing end-to-end business processes with a focus on domain rules, data integrity, and user experience.",
    technologies: [
      "Next.js",
      "TypeScript",
      "Tailwind CSS",
      "Drizzle ORM",
      "PostgreSQL",
    ],
  },
];
